...
## Added UI: Structure Loot + Position Picker

This workspace now includes a Swing UI entrypoint at `GPULootSeedFinder.Main`:

- `Ctrl + click` on structure cards to multi-select structures.
- Single click selects one structure.
- Selected structures show:
  - loot sample preview
  - decoration salt status
  - resolved structure positions (for supported structures like ruined portal)
- Texture support:
  - `Download Textures` downloads structure icons from internet URLs
  - textures are cached in `~/.gpulootseedfinder/textures`

### Run

1. Ensure JDK 16+ is installed and `JAVA_HOME` is set.
2. Build with Maven:
...
3. Launch the UI:
- From IDE: run `GPULootSeedFinder.Main`
- Or run shaded jar from `target/` (after `package`)
